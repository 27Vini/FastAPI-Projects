...
class ProductUseCases:
    ...
    def add_product(self,product : Product, category_slug : str):
        ...
        if not category:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f'No category was found with slig {category_slug}')
    ...
    def update_product(self,id : int, product : Product):
        ...
        if(product_on_db is None):
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail='Product with inputed id not found')
    ...
    def delete_product(self,id : int):
        product_on_db = self.db_session.query(ProductModel).filter_by(id=id).first()
        if product_on_db is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Product not found with provided id')
